...
static int createTable(LindormContest::TSDBEngine* engine) {
    ...
    // schema1.columnTypeMap["t1c4"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c5"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c6"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c7"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c8"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c9"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c10"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c11"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c12"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c13"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c14"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c15"] = LindormContest::COLUMN_TYPE_STRING;
    // schema1.columnTypeMap["t1c16"] = LindormContest::COLUMN_TYPE_STRING;
...
static void prepareStaticVariables() {
    ...
    row1.columns.insert(
        std::make_pair("t1c3", LindormContest::ColumnValue(str1, 20)));
    ...
    row3.columns.insert(
        std::make_pair("t1c3", LindormContest::ColumnValue(str2, 19)));
}
...
static int verifyTableData(LindormContest::TSDBEngine* engine) {
    ...
    if (r0.columns["t1c2"].getDoubleFloatValue(doubleBuff) != 0 ||
        doubleBuff != 102.1) {
        // std::cerr << "RES incorrect" << std::endl;
        assert(0);
        perror("RES incorrect");
    ...
    if (r0.columns["t1c3"].getStringValue(strBuff) != 0 ||
        strBuff.first != 19 || std::strncmp(strBuff.second, str2, 19) != 0) {
        // std::cout << std::string(str2, 19) << std::endl;
        // std::cerr << "RES incorrect" << std::endl;
        assert(0);
        perror("RES incorrect");
        return -1;
    ...
    if (r1.columns["t1c2"].getDoubleFloatValue(doubleBuff) != 0 ||
        doubleBuff != 101.1) {
        // std::cerr << "RES incorrect" << std::endl;
        perror("RES incorrect");
        return -1;
    ...
    if (r1.columns["t1c3"].getStringValue(strBuff) != 0 ||
        strBuff.first != 20 || std::strncmp(strBuff.second, str1, 20) != 0) {
        // std::cerr << "RES incorrect" << std::endl;
        perror("RES incorrect");
        return -1;
    }
...
int main() {
...
#elif TEST_MODE == 2
    LindormContest::TSDBEngine* engine =
        new LindormContest::TSDBEngineImpl0(dbPath.string());
#endif
    int ret;
    ret = engine->connect();
    if (ret != 0) {
    ...
    std::cout << "PASSED data verification before we restart the db"
              << std::endl;
    ...
    std::cout << "PASSED data verification after we restart the db"
              << std::endl;
