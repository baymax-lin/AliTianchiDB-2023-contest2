 ...
 * For the sake of simplicity, we only support AVG and MAX for this contest.
 <br>
 * The data type of the aggregation result is assumed as follows for simplicity:
 <br>
 * - AVG: DOUBLE <br>
 *        If all row were filtered, the result should be NaN (Bits as long:
 0xfff0000000000000L).<br>NaN
 *        You can get the NaN value by the following steps:<br><br>
 *            static int64_t DOUBLE_NAN_AS_LONG = 0xfff0000000000000L;<br>
              static double_t DOUBLE_NAN = * (double_t*)
 (&DOUBLE_NAN_AS_LONG);<br><br>
...
/**
 * Request for aggregation of some specified columns of this vin within a time
 * range. <br> timeLowerBound is included, timeUpperBound is excluded
 * [timeLowerBound, timeUpperBound).<br> "aggregator" means the aggregator
 * type.<br> "columnName" means the column name for aggregation.
 */
...
/**
 * Request for dividing the data within a specified time range for a given
 * vehicle (represented by Vin) into multiple windows based on a specified
 * interval and calculating the aggregation result of requested columns for each
 * window. <br> timeLowerBound is included, timeUpperBound is excluded
 * [timeLowerBound, timeUpperBound).<br> "interval" means the downsample
 * interval for each aggregation.<br> "columnFilter" means the value compare
 * expression for filtering rows, the filter is applied to the requested column
 * and takes effect during the aggregation phase only.
 */
