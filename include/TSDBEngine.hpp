...
class TSDBEngine {
   public:
    ...
    explicit TSDBEngine(std::string dataDirPath)
        ...
        if (ret != 0) {
            std::cerr << "Cannot get the status of path: " << this->dataDirPath
                      << std::endl;
            std::terminate();
        }
        ret = S_ISDIR(pathStat.st_mode);
        if (ret == 0) {
            std::cerr << "The data path is not a directory: "
                      << this->dataDirPath << std::endl;
            std::terminate();
        }
    }
     ...
     * @return 0 if success.
     */
    virtual int createTable(const std::string& tableName,
     ...
     * 2. If a row for the same vin and same timestamp exists, the action is
     * undefined.<br>
     ...
     * 3. If no column is requested (pReadReq.requestedColumns.empty() == true),
     ...
     */
    virtual int executeLatestQuery(const LatestQueryRequest& pReadReq,
                                   std::vector<Row>& pReadRes) = 0;

    /**
     * Read rows related to the vin from trReadReq and with a timestamp
     * [timeLowerBound, timeUpperBound). timeLowerBound is included, and
     * timeUpperBound is excluded. Put the results to trReadRes and return the
     * appended rows' num.<br> Attention:<br>
     * 1. Any implements of this function should be multi-thread friendly.<br>
     * 2. Do not return any column that was not requested.<br>
     * 3. If no column is requested (trReadReq.requestedColumnFieldNames.empty()
     * == true), return all columns.
     * @return 0 if read successfully.<br>
     *         if this vin doesn't exist or no values in the timestamp range
     * should also return 0.<br> others means error, under which the trReadRes
     * parameter is modified or not is undefined.
     */
    virtual int executeTimeRangeQuery(const TimeRangeQueryRequest& trReadReq,
                                      std::vector<Row>& trReadRes) = 0;

    /**
     * Calculate the aggregation result of requested columns for the specified
     * time range of a given vehicle (represented by vin). <br><br> For
     * instance, given a series of data in 10 seconds for a vin: <br> Assuming
     * that we do aggregation to column named "AZKP": <br> timestamp(ms)  | AZKP
     * <br>
     * ---------------|------- <br>
     * 1693274400000  | 10     <br>
     * 1693274401000  | 11     <br>
     ...
     * 1693274409000  | 16     <br>
     * <br>
     * If a TimeRangeAggregationRequest with the following properties received:
     * <br>
     ...
     *
     * @note <br>
     * 1. The names in the Map<String, ColumnValue> of the returned row should
     * be the same as the requested column name. <br>
     * 2. The timestamp in the returned row should be the start timestamp of the
     * given time range. <br>
     * 3. If there is a row whose timestamp equals upperBound, this row should
     * not be included.<br>
     * 4. If there is a row whose timestamp equals lowerBound, this row should
     * be included.<br>
     ...
     *         there should still be one row in the result vector and the return
     * value should still be 0.
     */
    virtual int executeAggregateQuery(
        const TimeRangeAggregationRequest& aggregationReq,
        std::vector<Row>& aggregationRes) = 0;

    /**
     * Divide the data within a specified time range for a given vehicle
     * (represented by Vin) into multiple windows based on a specified interval.
     ...
     * 1693274409000  | 16     <br>
     * <br>
     * If a TimeRangeDownsampleRequest with the following properties received,
     * <br>
     * - interval = 5000 ms <br>
     ...
     * <br>
     * On the other hand, if a TimeRangeDownsampleRequest with the following
     ...
     * <br>
     * Since no row is in such timestamp section, an EMPTY list would be
     * returned (Not NaN).<br> <br>
     *
     * @note <br>
     * 1. The names in the Map<String, ColumnValue> of the returned rows should
     * be the same as the requested column name.<br>
     * 2. For the purpose of simplicity, (timeUpperBound - timeLowerBound) is
     * guaranteed to be divisible by interval. <br>
     * 3. If there is a row whose timestamp equals upperBound, this row should
     * not be included.<br>
     * 4. If there is a row whose timestamp equals lowerBound, this row should
     * be included.<br>
     ...
     */
    virtual int executeDownsampleQuery(
        const TimeRangeDownsampleRequest& downsampleReq,
        std::vector<Row>& downsampleRes) = 0;
     ...
     */
    const std::string& getDataPath() { return dataDirPath; }

   protected:
    /**
     * The data path that all data stored. No other directory should be
     * written when the Database is running, or you will be called for a
     * foul.<br> Attention:<br>
     *   1. the dataPath targets a directory.<br>
     *   2. the targeted directory has been created before the constructor is
     * called.
     */
