...
static void* malloc(size_t size) {
    ...
    if (ptr == nullptr) {
        std::cerr << "Cannot allocate " << size << " bytes direct memory"
                  << std::endl;
        throw std::bad_alloc();
    }
    return ptr;
...
ColumnValue::ColumnValue()
    : columnType(COLUMN_TYPE_UNINITIALIZED), columnData(nullptr) {}

ColumnValue::ColumnValue(int32_t value) : columnType(COLUMN_TYPE_INTEGER) {
...
ColumnValue::ColumnValue(const ColumnValue& rhs) : columnType(rhs.columnType) {
    int32_t dataSize = rhs.getRawDataSize();
    columnData = (char*)malloc((size_t)dataSize);
    std::memcpy(columnData, rhs.columnData, dataSize);
}

ColumnValue::ColumnValue(ColumnValue&& rhs) noexcept
    : columnType(COLUMN_TYPE_UNINITIALIZED), columnData(nullptr) {
...
int ColumnValue::getStringValue(
    std::pair<int32_t, const char*>& lengthStrPair) const {
